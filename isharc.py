...
__update_notes__="""
1.4.0
    -   Fixed issues with confidence, allowing for multiple low value removal
        rather than single lowest (See flag [-dg]).
...
1.3.0
    -   Separated options for subtraction of "all" DG or the lowest
        confidence DG for coverage updating. 
...
def coverage_isolation(gene_coverage, bedpe_dict, 
    ...
    # print(gene_coverage) # Check the updated dictionary.
    return(gene_coverage)
...
def cov_to_bed(gene_coverage, gene_regions, output):
    ...
    with open(outfile, 'w') as f:
        f.write('track type=bedGraph visibility=full color=255,0,0 \
            altColor=0,0,255, priority=20, viewLimits=0:1000\n')
        for row in bedgraph:
...
def main():
    ...
    if '_pri_crssant_' in bedpe: 
        output = re.split(r'_pri_crssant_[^_]*dg.bedpe', bedpe)[0]
    else:
